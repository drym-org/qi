...
@bold{Common example}: Attempting to employ a @emph{Racket} expression producing multiple values in an expression position, e.g. @racket[(~> () (gen (values 1 2 3)) +)]. Whether the expression is Racket or Qi, the number of values returned must be the number of values expected -- and typically, that's @seclink["values-model" #:doc '(lib "scribblings/reference/reference.scrbl")]{one}. In this example, if you must use a Racket expression to produce the values, then you could use @racket[(~> () (esc (λ _ (values 1 2 3))) +)], instead.
